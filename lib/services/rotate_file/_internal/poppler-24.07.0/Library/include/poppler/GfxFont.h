...
public:
    ...
    locateFont(XRef *xref, PSOutputDev *ps, GooString *substituteFontName = nullptr);
    ...
    // the char code.
    virtual int
    getNextChar(const char *s, int len, CharCode *code, Unicode const **u, int *uLen, double *dx,
                double *dy, double *ox, double *oy) const = 0;
...
protected:
    GfxFont(const char *tagA, Ref idA, std::optional<std::string> &&nameA, GfxFontType typeA,
            Ref embFontIDA);
...
public:
    Gfx8BitFont(XRef *xref, const char *tagA, Ref idA, std::optional<std::string> &&nameA,
...
public:
    GfxCIDFont(XRef *xref, const char *tagA, Ref idA, std::optional<std::string> &&nameA,
               GfxFontType typeA, Ref embFontIDA, Dict *fontDict);
