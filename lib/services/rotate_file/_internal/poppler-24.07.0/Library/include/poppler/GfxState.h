...
public:
    ...
    GfxColorTransform(void *transformA, int cmsIntent, unsigned int inputPixelType,
...
public:
    ...
    static GfxColorSpace *
    parse(GfxResources *res, Object *csObj, OutputDev *out, GfxState *state, int recursion = 0);
    ...
    virtual void getGrayLine(unsigned char * /*in*/, unsigned char * /*out*/, int /*length*/) {
        error(errInternal, -1, "GfxColorSpace::getGrayLine this should not happen");
    }

    virtual void getRGBLine(unsigned char * /*in*/, unsigned int * /*out*/, int /*length*/) {
        error(errInternal, -1, "GfxColorSpace::getRGBLine (first variant) this should not happen");
    }

    virtual void getRGBLine(unsigned char * /*in*/, unsigned char * /*out*/, int /*length*/) {
        error(errInternal, -1, "GfxColorSpace::getRGBLine (second variant) this should not happen");
    }

    virtual void getRGBXLine(unsigned char * /*in*/, unsigned char * /*out*/, int /*length*/) {
        error(errInternal, -1, "GfxColorSpace::getRGBXLine this should not happen");
    }

    virtual void getCMYKLine(unsigned char * /*in*/, unsigned char * /*out*/, int /*length*/) {
        error(errInternal, -1, "GfxColorSpace::getCMYKLine this should not happen");
    }

    virtual void getDeviceNLine(unsigned char * /*in*/, unsigned char * /*out*/, int /*length*/) {
        error(errInternal, -1, "GfxColorSpace::getDeviceNLine this should not happen");
    }

    // create mapping for spot colorants
    virtual void
    createMapping(std::vector<GfxSeparationColorSpace *> *separationList, int maxSepComps);
...
public:
    ...
    static GfxColorSpace *
    parse(GfxResources *res, Array *arr, OutputDev *out, GfxState *state, int recursion);
    ...
    void createMapping(std::vector<GfxSeparationColorSpace *> *separationList,
                       int maxSepComps) override {
...
public:
    ...
    void
    createMapping(std::vector<GfxSeparationColorSpace *> *separationList, int maxSepComps) override;
...
private:
    GfxSeparationColorSpace(GooString *nameA, GfxColorSpace *altA, Function *funcA,
                            bool nonMarkingA, unsigned int overprintMaskA, int *mappingA);
...
public:
    GfxDeviceNColorSpace(int nCompsA, std::vector<std::string> &&namesA, GfxColorSpace *alt,
                         Function *func, std::vector<GfxSeparationColorSpace *> *sepsCS);
...
private:
    GfxDeviceNColorSpace(int nCompsA, const std::vector<std::string> &namesA, GfxColorSpace *alt,
                         Function *func, std::vector<GfxSeparationColorSpace *> *sepsCSA,
...
public:
    ...
    static GfxPattern *
    parse(GfxResources *res, Object *obj, OutputDev *out, GfxState *state, int patternRefNum);
...
private:
    GfxTilingPattern(int paintTypeA, int tilingTypeA, const double *bboxA, double xStepA,
                     double yStepA, const Object *resDictA, const double *matrixA,
...
public:
    GfxUnivariateShading(int typeA, double t0A, double t1A,
                         std::vector<std::unique_ptr<Function>> &&funcsA, bool extend0A,
                         bool extend1A);
    ...
    virtual void
    getParameterRange(double *lower, double *upper, double xMin, double yMin, double xMax,
                      double yMax) = 0;
...
public:
    ...
    static GfxFunctionShading *
    parse(GfxResources *res, Dict *dict, OutputDev *out, GfxState *state);
...
public:
    GfxAxialShading(double x0A, double y0A, double x1A, double y1A, double t0A, double t1A,
    ...
    void getParameterRange(double *lower, double *upper, double xMin, double yMin, double xMax,
...
public:
    GfxRadialShading(double x0A, double y0A, double r0A, double x1A, double y1A, double r1A,
    ...
    void
    getCoords(double *x0A, double *y0A, double *r0A, double *x1A, double *y1A, double *r1A) const {
        *x0A = x0;
        *y0A = y0;
        *r0A = r0;
        *x1A = x1;
...
public:
    GfxGouraudTriangleShading(int typeA, GfxGouraudVertex *verticesA, int nVerticesA,
    ...
    parse(GfxResources *res, int typeA, Dict *dict, Stream *str, OutputDev *out, GfxState *state);
     ...
     */
    void getTriangle(int i, double *x0, double *y0, GfxColor *color0, double *x1, double *y1,
                     GfxColor *color1, double *x2, double *y2, GfxColor *color2);
     ...
     * @precondition isParameterized() == true
     */
    void getTriangle(int i, double *x0, double *y0, double *color0, double *x1, double *y1,
...
public:
    ...
    bool useRGBLine() const {
        return (colorSpace2 && colorSpace2->useGetRGBLine()) ||
               (!colorSpace2 && colorSpace->useGetRGBLine());
    }

    bool useCMYKLine() const {
        return (colorSpace2 && colorSpace2->useGetCMYKLine()) ||
               (!colorSpace2 && colorSpace->useGetCMYKLine());
    }
...
private:
    ...
    GfxPath(bool justMoved1, double firstX1, double firstY1, GfxSubpath **subpaths1, int n1,
            int size1);
};
