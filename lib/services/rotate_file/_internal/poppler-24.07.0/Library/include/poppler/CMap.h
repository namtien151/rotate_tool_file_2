...
public:
    ...
    parse(CMapCache *cache, const GooString *collectionA, Object *obj);

    // Create the CMap specified by <collection> and <cMapName>.  Sets
    // the initial reference count to 1.  Returns NULL on failure.
    static std::shared_ptr <CMap>
    parse(CMapCache *cache, const GooString *collectionA, const GooString *cMapNameA);

    // Parse a CMap from <str>.  Sets the initial reference count to 1.
    // Returns NULL on failure.
    static std::shared_ptr <CMap>
    parse(CMapCache *cache, const GooString *collectionA, Stream *str);
...
private:
    ...
    void setReverseMapVector(unsigned int startCode, CMapVectorEntry *vec, unsigned int *rmap,
                             unsigned int rmapSize, unsigned int ncand);
