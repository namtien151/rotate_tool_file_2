...
struct Ref {
    ...
    static constexpr Ref

    INVALID() { return {-1, -1}; };
};

inline bool operator==(const Ref lhs, const Ref rhs)
...
noexcept
{
return lhs.num != rhs.num || lhs.gen != rhs.
gen;
}
...
namespace std {
    ...
    struct hash<Ref> {
        ...
        noexcept { return std::hash<int>{}(ref.num) ^ (std::hash<int>{}(ref.gen) << 1); }
    };
