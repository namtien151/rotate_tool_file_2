...
import '../../../services/serverces_1/shell_process.dart';
import '../../widget/normal/notify/show_notify.dart';
import 'package:path/path.dart' as path;
import 'package:http/http.dart' as http;
class RotateToolViewmodel extends BaseViewModel {
  ...
  RotateToolViewmodel(this.context) {
  ...
  Future<void> rotateFiles() async {
    ...
    await _startUploadTasks(totalFiles);
  }

  Future<void> _startUploadTasks(int totalFiles) async {
    final int maxConcurrentUploads =
    _selectedFileCount!; // Số lượng file xử lý đồng thời
    List<Future<void>> uploadTasks = [];

    while (fileQueue.isNotEmpty) {
      // Thêm tác vụ upload mới vào danh sách
      uploadTasks.add(_startNextUploadTask(totalFiles));

      // Giới hạn số lượng tác vụ đang chạy
      if (uploadTasks.length >= maxConcurrentUploads) {
        await Future.wait(uploadTasks); // Chờ cho tất cả tác vụ hoàn thành
        uploadTasks.clear(); // Xóa danh sách tác vụ đã hoàn thành
      }
    }

    // Chờ tất cả các tác vụ còn lại
  ...
  Future<void> _uploadFileWithIsolate(
    ...
    // Kiểm tra chỉ số hợp lệ trước khi bắt đầu
    if (index < 0 || index >= fileStatuses.length) {
      print("Invalid index: $index for fileStatuses.");
      semaphore.release(); // Giải phóng semaphore nếu chỉ số không hợp lệ
      return; // Kết thúc hàm nếu chỉ số không hợp lệ
    }

    final receivePort = ReceivePort();
    await Isolate.spawn(_uploadIsolateEntryPoint, [file.path, receivePort.sendPort]);
    print('Bắt đầu xử lý file: $file tại thời điểm: ${DateTime.now()}');

    receivePort.listen((message) async {
      if (message is Map) {
        // Kiểm tra chỉ số hợp lệ tại đây
        if (index < 0 || index >= fileStatuses.length) {
          print("Invalid index: $index for fileStatuses.");
          semaphore.release(); // Giải phóng semaphore nếu chỉ số không hợp lệ
          return; // Kết thúc hàm nếu chỉ số không hợp lệ
        ...
        if (message.containsKey('progress')) {
          ...
          if (message['status'] == 'done') {
            try {
              ...
              // Cập nhật trạng thái lỗi nếu có vấn đề khi lưu file
              fileStatuses[index].status = FileStatusType.idle; // Đặt trạng thái lỗi
              notifyListeners();
            }
          } else if (message['status'] == 'failed') {
            fileStatuses[index].status = FileStatusType.idle; // Đặt trạng thái là lỗi
  ...
  void clearAll() {
    ...
    shellProcess.stopExeFile(); // Gọi hàm dừng exe
